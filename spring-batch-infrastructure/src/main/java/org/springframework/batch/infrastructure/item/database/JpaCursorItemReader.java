...
/**
 * {@link ItemStreamReader} implementation based on JPA {@link Query#getResultStream()}.
 * It executes the JPQL query when initialized and iterates over the result set as
 * {@link #read()} method is called, returning an object corresponding to the current row.
 * The query can be set directly using {@link #setQueryString(String)}, or using a query
 * provider via {@link #setQueryProvider(JpaQueryProvider)}.
 * <p>
