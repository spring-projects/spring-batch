...
public class DerbyPagingQueryProviderTests extends AbstractSqlPagingQueryProviderTests {
	...
	public String getFirstPageSqlWithoutSortKeys() {
		return "SELECT * FROM ( SELECT TMP_ORDERED.*, ROW_NUMBER() OVER () AS ROW_NUMBER FROM (SELECT id, name, age FROM foo WHERE bar = 1 ) AS TMP_ORDERED) AS TMP_SUB WHERE TMP_SUB.ROW_NUMBER <= 100";
	}
}
