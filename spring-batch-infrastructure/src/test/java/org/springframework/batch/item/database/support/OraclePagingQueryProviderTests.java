...
public class OraclePagingQueryProviderTests extends AbstractSqlPagingQueryProviderTests {
	...
	public void testGenerateJumpToItemQuery() {
		String sql = "SELECT * FROM (SELECT id, ROWNUM as TMP_ROW_NUM FROM (SELECT id FROM foo WHERE bar = 1 ORDER BY id ASC)) WHERE TMP_ROW_NUM = 100";
	...
	public void testGenerateJumpToItemQueryForFirstPage() {
		String sql = "SELECT * FROM (SELECT id, ROWNUM as TMP_ROW_NUM FROM (SELECT id FROM foo WHERE bar = 1 ORDER BY id ASC)) WHERE TMP_ROW_NUM = 1";
