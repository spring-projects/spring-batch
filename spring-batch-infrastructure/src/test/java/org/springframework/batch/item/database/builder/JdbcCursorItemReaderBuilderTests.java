...
public class JdbcCursorItemReaderBuilderTests {
    ...
    public void testSimpleScenario() throws Exception {
        JdbcCursorItemReader<Foo> reader = new JdbcCursorItemReaderBuilder<Foo>()
                ...
                .name(READER_INSTANCE_NAME)
                .sql(SELECT_QUERY)
                .rowMapper((rs, rowNum) -> {
                    Foo foo = new Foo();

                    foo.setFirst(rs.getInt(COLUMN_FIRST));
                    foo.setSecond(rs.getString(COLUMN_SECOND));
                    foo.setThird(rs.getString(COLUMN_THIRD));
    ...
    public void testQueryArgumentsList() throws Exception {
        JdbcCursorItemReader<Foo> reader = new JdbcCursorItemReaderBuilder<Foo>()
                .dataSource(this.dataSource)
                .name(READER_INSTANCE_NAME)
                .sql(PARAMETERIZED_SELECT_QUERY)
                .queryArguments(Collections.singletonList(3))
                .rowMapper((rs, rowNum) -> {
                    Foo foo = new Foo();
