...
public interface StepExecutionSplitter {
	 ...
	 * are going to execute a the same step for each execution context in the partition.
	 * @return the name of the step that will execute the business logic
	 */
	String getStepName();

	/**
	 * Partition the provided {@link StepExecution} into a set of parallel executable
	 * instances with the same parent {@link JobExecution}. The grid size will be treated
	 * as a hint for the size of the collection to be returned. It may or may not
	 * correspond to the physical size of an execution grid.<br>
	 * <br>
	 *
	 * On a restart clients of the {@link StepExecutionSplitter} should expect it to
	 * reconstitute the state of the last failed execution and only return those
	 * executions that need to be restarted. Thus the grid size hint will be ignored on a
	 * restart.
