...
public class StopAndRestartFailedJobParserTests extends AbstractJobParserTests {
	...
	public void testStopRestartOnCompletedStep() throws Exception {
		...
		// First Launch
		//
		JobExecution jobExecution = launchAndAssert("[job.s0, job.s1]");
		StepExecution stepExecution = getStepExecution(jobExecution, "job.s1");
		...
		//
		stepNamesList.clear();
		jobExecution = launchAndAssert("[job.s0, job.s2]");
		stepExecution = getStepExecution(jobExecution, "job.s2");
