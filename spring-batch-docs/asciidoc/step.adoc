...
in the class hierarchy. Any unclassifed exception is treated as 'fatal'.
...
`BatchStatus` of `FAILED`. Unlike end, the failing a `Job` does not prevent the `Job`
...
[role="javaContent"]
When using java configuration, the `stopAndRestar` method requires a 'restart' attribute
...
time in a linear fashion. In addition to this typical style, the Spring Batch also allows
...
elements, such as the 'next' attribute or the 'next', 'end', 'fail', or 'pause' elements.
