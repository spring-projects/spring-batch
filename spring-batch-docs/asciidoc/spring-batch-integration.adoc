...
public IntegrationFlow jmsReplies() {
    return IntegrationFlows.from(Jms.inboundAdapter(connectionFactory())
                    .configureJmsTemplate(t ->
                            t.deliveryPersistent(true).jmsMessageConverter(new SimpleMessageConverter()))
                    .destination("replies"), c -> c.poller(Pollers.fixedRate(1000).maxMessagesPerPoll(1)))
            .channel(replies())
            .get();
}
----
...
public IntegrationFlow jmsIn() {
    return IntegrationFlows.from(Jms.inboundAdapter(connectionFactory())
                    ...
                    .destination("requests"), c -> c.poller(Pollers.fixedRate(1000).maxMessagesPerPoll(1)))
            .channel(requests())
            .get();
}
...
public MessageHandler serviceActivator() throws Exception{
...
public IntegrationFlow inboundJmsRequests() {
    return IntegrationFlows.from(Jms.inboundAdapter(connectionFactory())
                    .configureJmsTemplate(t -> t.deliveryPersistent(true)
                            .jmsMessageConverter(new SimpleMessageConverter()))
                    .destination("requestsQueue"), c -> c.poller(Pollers.fixedRate(1000).maxMessagesPerPoll(1)))
...
public IntegrationFlow inboundJmsStaging() {
    return IntegrationFlows.from(Jms.inboundAdapter(connectionFactory())
                    .configureJmsTemplate(t -> t.deliveryPersistent(true)
                            .jmsMessageConverter(new SimpleMessageConverter()))
                    .destination("stagingQueue"), c -> c.poller(Pollers.fixedRate(1000).maxMessagesPerPoll(1)))
