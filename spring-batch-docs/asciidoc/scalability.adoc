...
The `Job` runs on the left-hand side as a sequence of `Step` implementations,
    and one of the `Step` implementations is labeled as a master. The slaves in this picture
...
There is a simple example that can be copied and extended in the
    unit test suite for https://github.com/spring-projects/spring-batch/tree/master/spring-batch-samples[Spring Batch Samples] (see
...
The `PartitionHandler` is the component that
      ...
      `Step` implementations, wrapped in some fabric-specific format, like a DTO. It does not
...
Spring Batch does, however, provide a useful implementation of
      `PartitionHandler` that executes `Step` implementations locally in
...
The `TaskExecutorPartitionHandler` is
      useful for IO-intensive `Step` implementations, such as copying large numbers of files or
