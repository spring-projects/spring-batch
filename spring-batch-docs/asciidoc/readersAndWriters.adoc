...
link:$$http://docs.spring.io/spring/docs/3.2.x/spring-framework-reference/html/resources.html$$[Spring
...
architecture only needs to kno how to locate the files to be processed. Spring Batch
...
attempting to access an index in a `RowMapper` of `FieldSetMapper` that does not exist or
...
the `ItemStream`, so you may need to register it as a stream in the configuration XML.
