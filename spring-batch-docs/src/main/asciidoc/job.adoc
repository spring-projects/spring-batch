...
how a `Job` can be run and how its meta-data can be
...
Noted that the `afterJob` method is called regardless of the success or
...
`Job` instances may inherit properties. Similar to class
...
method is `SERIALIZABLE`, which is quite aggressive. `READ_COMMITTED` usually works just as
...
time. Another reason may be that you do not need to persist status for a particular
...
Note that the in-memory repository is volatile and, thus, does not allow restarting between JVM
instances. It also cannot guarantee that two job instances with the same parameters are
launched simultaneously. It is also not suitable for use in a multi-threaded `Job` or a locally
...
it is often be necessary to override the
...
you run from within a web container within the scope of an
...
All of these tasks are accomplished using only the arguments passed in.
...
for simplicity, the class was used directly. This example uses the same `EndOfDay`
...
NOTE: If you set the table prefix on the job repository, don't forget to set it on the job explorer as well.
...
ifdef::backend-pdf[]
You can associate an incrementer can
be associated with a `Job` by using the `incrementer`
