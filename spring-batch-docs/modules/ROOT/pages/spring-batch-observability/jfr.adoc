...
JFR can be enabled by registering a `FlightRecorderBatchEventRecorder` in the application context:

[source, java]
----
@Bean
public BatchEventRecorder batchEventRecorder() {
	return new FlightRecorderBatchEventRecorder();
}
----

Start the application with recording enabled to capture the events:
