...
[[statelessSessionItemWriter]]
=== `StatelessSessionItemWriter`
The `StatelessSessionItemWriter` is an `ItemWriter` that uses a Hibernate `StatelessSession`
to insert of all items provided. Spring Batch provides a `StatelessSessionItemWriterBuilder`
to construct an instance of the `StatelessSessionItemWriter`.
...
[[resourcesItemReader]]
=== `ResourcesItemReader`
The `ResourcesItemReader` serves up `Resource` items themselves, instead of their
content, one `Resource` per `read`. It is typically used as the reader of a step that
processes each matching resource individually (for example, an archiving or file-moving
step). Spring Batch provides a `ResourcesItemReaderBuilder` to construct an instance of
the `ResourcesItemReader`, which can resolve resources directly from one or more location
patterns:
...
in addition to Ant-style wildcards (`*`, `**`, and `?`).
