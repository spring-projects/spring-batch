...
in your MongoDB database before running any job. The provided MongoDB schema scripts are idempotent and
can be safely reapplied without resetting sequence values.
...
[[repositoryCollectionPrefix]]
== Changing the Collection Prefix

The MongoDB job repository offers the same capability for the metadata collections. By
default, they are all prefixed with `BATCH_`: `BATCH_JOB_INSTANCE`,
`BATCH_JOB_EXECUTION`, `BATCH_STEP_EXECUTION` and `BATCH_SEQUENCES`. The prefix can be
changed to run several applications against the same MongoDB database, or to isolate
...
Given the preceding change, the metadata is stored in the `MY_APP_JOB_INSTANCE`,
`MY_APP_JOB_EXECUTION`, `MY_APP_STEP_EXECUTION` and `MY_APP_SEQUENCES` collections. The
identifiers of the sequence documents held in the sequences collection are prefixed as
well, so `MY_APP_SEQUENCES` contains `MY_APP_JOB_INSTANCE_SEQ`,
`MY_APP_JOB_EXECUTION_SEQ` and `MY_APP_STEP_EXECUTION_SEQ`.

IMPORTANT: The `org/springframework/batch/core/schema-mongodb.jsonl` script that creates
the collections and seeds the sequence documents uses the default prefix. When you
configure a custom prefix, you need to adapt that script accordingly, just as you would
adapt the DDL scripts for a custom table prefix.

NOTE: Only the collection prefix is configurable. The collection and field names are not.
