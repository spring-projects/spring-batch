...
A flow is a sequence of steps, together with the transitions between them, that can be
executed as a job or as part of a job. A split lets several flows run in parallel.
...
following example shows, a `Flow` can be built on its own with a `FlowBuilder` by using
the same `start` and `next` methods as when defining a job. In the job definition, `split`
and `add` put `flow1` (the flow that the job starts with) and `flow2` into a split, which
runs them with the `TaskExecutor` passed to `split`. `next` then defines what runs after
the split. The job definition is itself a flow, which `end` completes before `build`
creates the `Job`.
+
[source, java]
----
...
The `task-executor` attribute references the `TaskExecutor` that runs the flows.
+
[source, xml]
----
...
In the preceding example, the two flows run in parallel: one runs `step1` and then `step2`,
while the other runs `step3`. `step4` starts only after both flows have completed. Note
that an asynchronous `TaskExecutor`, such as `SimpleAsyncTaskExecutor`, is required to run
the flows in parallel.
