...
|`JpaItemWriter`|Writes items through the transactional JPA `EntityManager`,
            calling `merge` (or `persist` when `usePersist` is set) on each
            item that is not already part of the persistence context. At the
            end of each chunk, it flushes the `EntityManager` and, unless
            `clearPersistenceContext` is `false`, clears it.|Yes
